...
/*
   ...
   * name: the name of the campground
    It returns the json object containing all the information for a campground name, location(town) and maxlength(lengthlimit)
...
app.get('/search', async (req, res) => {
    ...
    // TODO
    try {
       const template = "SELECT name AS campground, location, maxLength FROM campgrounds WHERE name = $1";
...
//something wrong with select statement, maxLength is not maintaining case. Output is maxlength need it to be maxLength
    try {
        const template = "SELECT name AS campground, location, maxlength AS \"maxLength\" FROM campgrounds WHERE maxlength >= $1";
        const dbresponse = await pool.query(template, [length]);
        const results = dbresponse.rows.map((row) => {return row});
...
app.get('/elevation', async (req, res) => {
    ...
    if (req.query.direction == 'lower'){
        query = "SELECT name AS campground, elevation, location AS town FROM campgrounds WHERE elevation < $1";
    } else {
        query =  "SELECT name AS campground, elevation, location AS town FROM campgrounds WHERE elevation > $1";
    }
    try {
        const dbresponse = await pool.query(query,[req.query.altitude]);
        const results = dbresponse.rows.map((row) => {return row});
        res.json({campgrounds: results})
...
app.post('/addCampground', async (req,res) => {
    ...
    const padType = req.body.padType;
    const sites = req.body.sites;
    try {
        const template1 = "SELECT * FROM campgrounds WHERE name = $1 AND location = $2 ";
        const check = await pool.query(template1, [name, town]);
        if (check.rowCount != 0){
            ...
            const response = await pool.query(template2, [name, town, maxlength, elevation, padType, sites]);
            res.json({status: "added"});
        }
    } catch (err){
        // whoops
